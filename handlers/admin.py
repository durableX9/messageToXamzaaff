	...
	try: 
		...
		if(db_profile_access(uid) >= 1):
			...
			if len(args) >= 2:
				...
				for ot in args:
					answer+=ot+" "
				await message.reply('Siz Muvafaqiyatli Savoliga Javob Berdingiz!')
				await bot.send_message(chatid, f"Yangi Habar!\nXamzaaff`dan javob:\n\n '{answer}'", parse_mode='Markdown')
				return
			else:
				await message.reply("⚠ Iltimos to'g'ri bering: \nПример: `/ответ 516712732 Ваш ответ`",parse_mode='Markdown')
				return
	else:
		return

async def admin_giveacces(message: types.Message):
	try:
